        ...
        <form method="get" id="internship-search-form" class="d-flex">
            <div class="input-group">
                ...
                @foreach(request()->query() as $param => $value)
                    ...
                    @endif
                @endforeach
                <button class="btn btn-outline-secondary" type="submit" id="internship-search-btn">
                    <i class="bi bi-search"></i>
                    <span class="spinner-border spinner-border-sm d-none" id="internship-search-spinner"></span>
                </button>
                @if(request('q'))
                <button class="btn btn-outline-secondary" type="button" id="internship-search-clear"><i class="bi bi-x"></i></button>
                @endif
            </div>
        </form>
...
var isForm = document.getElementById('internship-search-form');
if(isForm){
    var isInput = document.getElementById('internship-search-input');
    var isBtn = document.getElementById('internship-search-btn');
    var isSpinner = document.getElementById('internship-search-spinner');
    var debounceTimer;
    isInput.addEventListener('input', function(){
        clearTimeout(debounceTimer);
        debounceTimer = setTimeout(function(){
            isForm.submit();
        }, 300);
    });
    isForm.addEventListener('submit', function(){
        isBtn.disabled = true;
        if(isSpinner) isSpinner.classList.remove('d-none');
    });
    var isClear = document.getElementById('internship-search-clear');
    if(isClear){
